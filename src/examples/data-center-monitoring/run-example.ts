...
async function main() {
  ...
  for (const seq of sequences) {
    ...
    if (validation.valid) {
      ...
      const initialVectors = vectors.filter(v => (v as any).isInitial);
  ...
  for (let i = 0; i < progressionVectors.length; i++) {
    ...
    const criticals = result.totalOutputs.filter((o: OutputVector) =>
      (o.metadata?.description as string)?.toLowerCase().includes('critical')
    );
    const emergencies = result.totalOutputs.filter((o: OutputVector) =>
    ...
    if (result.totalOutputs.length > 0) {
      ...
      if (emergencies.length > 0) {
        ...
        emergencies.forEach((output: OutputVector) => {
          console.log(`      ${output.metadata?.description || 'Unknown'}`);
        });
      } else if (criticals.length > 0) {
        console.log(`   ⚠️  CRITICAL OUTPUTS:`);
        criticals.length > 0 && criticals.forEach((output: OutputVector) => {
          console.log(`      ${output.metadata?.description || 'Unknown'}`);
        });
      }
    }
