...
import { generateInputVectors } from './input-patterns.js';
...
export function generateDemoDataset(): DemoDataset {
  ...
  return {
    ...
    metadata: {
      ...
      totalVectors: sequences.reduce((sum, seq) => sum + seq.getAllVectors().length, 0),
...
function generateIndustrialSequences(): CriticalEventSequence[] {
  ...
  const industNames = ['Chemical Process Control', 'Equipment Monitor', 'Supply Chain Tracker', 'Bottleneck Detection'];
  for (let i = 0; i < 4; i++) {
    const seq = new CriticalEventSequence(industNames[i] ?? 'Unknown Sequence');
...
function generateHealthcareSequences(): CriticalEventSequence[] {
  ...
  const healthNames = ['Outbreak Detection System', 'Medical Equipment Alert', 'Treatment Progress Tracker'];
  for (let i = 0; i < 3; i++) {
    const seq = new CriticalEventSequence(healthNames[i] ?? 'Unknown Sequence');
    const vec = new RealityVector(
...
function generateSecuritySequences(): CriticalEventSequence[] {
  ...
  const securityNames = ['Access Pattern Analyzer', 'Intrusion Detection System', 'Network Security Monitor'];
  for (let i = 0; i < 3; i++) {
    const seq = new CriticalEventSequence(securityNames[i] ?? 'Unknown Sequence');
    const vec = new RealityVector(
      Array(3).fill(null).map(() => ({ value: 0.5, comparatorType: ComparatorType.THRESHOLD, threshold: 0.2 })),
      true
    );
...
function generateFinancialSequences(): CriticalEventSequence[] {
  ...
  for (let i = 0; i < 4; i++) {
    const seq = new CriticalEventSequence(names[i] ?? 'Unknown Sequence');
    const vec = new RealityVector(
    ...
    if (output) {
      vec.addOutputVector(createOutputVector(output, seq.id));
    }
    seq.addVector(vec);
    sequences.push(seq);
  }
